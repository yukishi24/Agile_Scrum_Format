...
create_issue_labels_from_json() {
  ...
  local total
  total=$(jq '.labels | length' "$LABEL_JSON")
  log "labels to create: $total"

  for i in $(seq 0 $((total - 1))); do
    local name color desc
    name=$(jq -r ".labels[$i].name" "$LABEL_JSON")
    color=$(jq -r ".labels[$i].color" "$LABEL_JSON")
    desc=$(jq -r ".labels[$i].description" "$LABEL_JSON")
    ...
    case "$http_code" in
      201)
        ...
        ;;
      422)
        log "  ⚠️  $name already exists (skip)"
        ;;
      *)
        log_error "  ❌ $name create failed (HTTP $http_code)"
        exit 1
        ;;
    esac
  done
